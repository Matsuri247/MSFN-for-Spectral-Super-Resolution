...
import os
from NTIRE2020_dataset import TrainDataset, ValidDataset
...
parser.add_argument("--end_epoch", type=int, default=5, help="number of epochs")
parser.add_argument("--init_lr", type=float, default=4e-4, help="initial learning rate")
parser.add_argument("--outf", type=str, default='./exp/MSFN/', help='path log files')
parser.add_argument("--data_root", type=str, default='../dataset/NTIRE2020')
...
def validate(val_loader, model):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        # target = target.cuda()
        _, _, h, w = input.shape
        input = input[:, :, h//2-128:h//2+128, w//2-128:w//2+128].cuda()
        target = target[:, :, h//2-128:h//2+128, w//2-128:w//2+128].cuda()
        with torch.no_grad():
            # compute output
            # To avoid out of memory, crop center 256*256 patch to validate
            output = model(input)
